"""
JMeter Performance Test Runner for PetClinic

This script:
1. Discovers all .jmx files in the current directory
2. Before running tests, resets database data using populate_test_data.py
...
print(f"\n{'='*70}")
print(f"PetClinic JMeter Performance Test Runner")
print(f"{'='*70}\n")
...
def reset_test_data(env_name):
    """Reset database using populate_test_data.py script"""
    test_data_script = Path(__file__).parent.parent / "test_data" / "populate_test_data.py"
    
    print(f"\n{'='*70}")
    print(f"Step 1: Resetting test data...")
    print(f"{'='*70}")
